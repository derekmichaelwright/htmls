...
xx <- data.frame(Group = c("X","X","Y","Y","Y","X","X","X","Y","Y"),
                 Data1 = 1:10, Data2 = seq(10, 100, by = 10))
...
xx <- xx %>%
  ...
  arrange(Location)
DT::datatable(xx)
xx <- xx %>% spread(Location, Mean_DTF)
DT::datatable(xx)
```
...
```{r}
xd$x11 <- NULL #creates and empty column
for(i in 1:nrow(xd)) {
  if(xd$x10[i] == T) { 
    xd$x11[i] <- "X" 
  } else{ xd$x11[i] <- "Y" }
...
xd$x11
xd <- xd %>% mutate(x11 = ifelse(x10 == T, "X", "Y"))
xd$x11
```
...
ggplot(yy, aes(x = Location, y = Mean_DTF, color = Name, group = Name)) +
