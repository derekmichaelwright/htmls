...
```
DTF in Saskatoon, Canada is significantly different from DTF in Jessore, Bangladesh.

```{r}
summary(aov(DTF ~ Name * Location, data = dd))
```
As expected, an ANOVA tells show statistical significance for genotype (\*), Location (\*\*\*) and GxE (\*\*\*).
