...
```{r echo = F}
anim_save("../../../myblog/content/graphs_agdata/global_food_security/gallery/gallery/global_food_security_gifs_01.gif", mp, width = 600, height = 400)
```
...
# Prep Data
xx <- dd %>% filter(Area != "World") %>%
  select(Area, Year, Region, Crop, TonnesPerPerson) %>%
  spread(Crop, TonnesPerPerson)
# Plot
mp <- ggplot(xx, aes(x = Cereals, y = Pulses)) + 
  geom_point(aes(size = Tubers, color = Region), alpha = 0.5) + 
...
```{r echo = F}
anim_save("../../../myblog/content/graphs_agdata/global_food_security/gallery/gallery/global_food_security_gifs_02.gif", mp, width = 600, height = 400)
```

![](global_food_security_gifs_02.gif)
