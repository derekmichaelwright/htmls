...
date: "2020-10-01"
author: "Derek Michael Wright <wrightmderek@gmail.com> [www.dblogr.com/](https://dblogr.com/) [-Image Gallery-](https://dblogr.com/graphs_agdata/import_export/)"
---
...
netImportExport_anim <- function(crop = "Wheat") {
  ...
  xx <- agData_FAO_Trade %>% addRegionInfo() %>%
    ...
    filter(!is.na(Net)) %>% arrange(Net) %>%
    mutate(Area = factor(Area, levels = rev(unique(.$Area))))
  x1 <- xx %>% group_by(Year) %>% top_n(n=5, Net) %>% mutate(Label = "Top 10") %>% 
    arrange(Net) %>% pull(Area) %>% as.character() %>% unique()
  x2 <- xx %>% group_by(Year) %>% top_n(n=5, -Net) %>% mutate(Label = "Bottom 10") %>% 
    arrange(Net) %>% pull(Area) %>% as.character() %>% unique()
  x1 <- setdiff(x1, x2)
  ...
  ggplot(xx, aes(x = Area, y = Net / 1000000, fill = Region)) + 
    ...
    scale_fill_manual(values = agData_Colors[c(1,4,3,2,7)]) +
    theme_agData(rotx = T) + 
    labs(title = crop, subtitle = "Export - Import {round(frame_time)}", 
